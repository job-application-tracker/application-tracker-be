const { shapeForJobData } = require('../../data/jobs');
...
module.exports = class Tracker {
  ...
  static async getAll(userId) {
    const { rows } = await pool.query(
      'SELECT id, user_id, position, company, status, created_at, index from trackers WHERE user_id=$1',
      [userId]
    );
  ...
  static async updateById(id, attrs) {
    ...
    const { rows } = await pool.query(
      `
    UPDATE trackers set position=$2, company=$3, description=$4, status=$5, notes=$6, applied_at=$7, interviewed_at=$8, closed_at=$9
    WHERE id=$1
    RETURNING *`,
      [
        id,
